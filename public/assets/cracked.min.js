!(function () {
      ...
      if (__.isStr(arguments[0])) {
        ...
        (_currentSelector = processSelectorForMacro(arguments[0])),
          ...
          _selectedNodes.forEach(function (el, i, arr) {
            el &&
              getNodeWithUUID(el).getMacroContainerUUID() !== macroUUID &&
              arr.splice(i, 1);
          });
      } else
        __.isObj(arguments[0]) &&
  ...
  function processSelectorForMacro(selector) {
    for (
      var selectorArr = selector.split(','),
        ...
        i = 0;
      i < selectorArr.length;
      i++
    )
      selectorArr[i] =
        -1 !== selectorArr[i].indexOf(prefix)
          ? selectorArr[i]
          : prefix + selectorArr[i];
    return selectorArr.join(',');
  }
  function reset() {
    (_previousNode = null), (_selectedNodes = []), (_currentSelector = '');
  }
  function resetSelection() {
    (_selectedNodes = []), (_currentSelector = '');
  }
  function createNode(type, creationParams, userSettings) {
    var node = new AudioNode(type, creationParams, userSettings || {});
    if (
      (saveNode(node),
      __.isFun(__.onCreateNode) &&
    ...
    return resetSelection(), setPreviousNode(node), node;
  }
  function audioNodeFactory(creationParams) {
    ...
    if (_context && creationParams.method && _context[creationParams.method]) {
      ...
      for (var creationParam in creationParams.settings)
        creationParams.settings.hasOwnProperty(creationParam) &&
          ('to_channel' === creationParam || 'from_channel' === creationParam
    ...
    } else if (_context && 'createDestination' === creationParams.method)
      node = _context.destination;
    else if (_context && 'createOrigin' === creationParams.method)
      node = createMockMediaStream(creationParams);
    else {
      if (!_context || 'createMacro' !== creationParams.method)
        throw new Error(
  ...
  function AudioNode(type, creationParams, userSettings) {
    ...
    function getRawNode(node, position) {
      if (__.isArr(node)) {
        var pos = position ? node.length - 1 : 0;
        ...
        if (__.isArr(rawNode)) {
          for (var i = 0; i < rawNode.length; i++)
            if (
              ...
              __.isNotUndef(rawNode[i][resolvedProperty]))
            ) {
          ...
          (resolvedProperty = resolveParamMapping(
            ...
            (rawNode = rawNode[resolvedProperty]
              ? rawNode[resolvedProperty]
              : null);
        return rawNode;
      }
      return null;
    ...
    var uuid,
      ...
      that = this;
    (uuid = generateUUID()),
      mergeObjects(userSettings, creationParams.settings),
      mergeObjects(userSettings.mapping, creationParams.mapping),
      (parameters = creationParams),
      (paramMapping =
        creationParams.mapping || creationParams.settings.mapping),
      (nodeType = type),
      (nativeNode = audioNodeFactory(parameters)),
      (nativeNode.uuid = uuid),
      (this.start = function (nodeParam) {
        ...
        else if (currNode && __.isFun(currNode.start)) {
          ...
          if (!wrapper.getIsPlaying()) {
            var offset =
              ...
              time =
                (currNode &&
                  currNode.loopEnd &&
                  __.isNum(currNode.loopEnd) &&
                  currNode.loopEnd,
                _ignoreGrid ? _context.currentTime : _loopTimeToNextStep);
            offset ? currNode.start(time, offset) : currNode.start(time),
              wrapper.setIsPlaying(!0),
              this.setIsPlaying(!0);
          }
        }
      }),
            ...
            if (
              ...
              for (var i = 0; i < target.length; i++)
                (prevTime = __.isUndef(time[i - 1])
                  ...
                  (_target =
                    0 === target[i] && 'exp' === type ? 1e-4 : target[i]),
              ...
              _ignoreGrid ||
                __.sec2ms(time) !== _loopInterval ||
                (time = Math.min(
                  time,
                  _loopTimeToNextStep - _context.currentTime
                )),
                logToConsole(
                  ' target ' +
                    target +
                ...
                (_target = 0 === target && 'exp' === type ? 1e-4 : target),
        ...
        if (str)
          for (
            var prefix = this.getMacroNameSpace(),
              selector = prefix + str,
              nodeIds = getNodesWithSelector(selector),
              i = 0;
            i < nodeIds.length;
            i++
          )
            getNodeWithUUID(nodeIds[i]).getMacroContainerUUID() ===
              this.getUUID() && nodes.push(nodeIds[i]);
        return nodes;
      }),
      ...
      (this.setMacroContainerUUID = function (uuid) {
        __.isStr(uuid) && (macroContainerUUID = uuid);
      }),
      ...
      (this.pushNextNode = function (node) {
        if (node) {
          var nodes = getNodesToConnect(this, node),
            fromNode = getNodeWithUUID(nodes.from.uuid);
          fromNode.getUUID() !== this.getUUID()
        ...
        return (
          !(
            ...
            !getNodeWithUUID(this.getMacroContainerUUID()).getParams().settings
              .modulates
          ) || !!this.getParams().settings.modulates
        );
      }),
      ...
      (this.replaceNode = function (node) {
        ...
        (nativeNode = node), (nativeNode.uuid = this.getUUID());
        for (var i = 0; i < nextNode.length; i++) this.connect(nextNode[i]);
        this.connectPrevious(),
          this.setIsPlaying(!1),
          saved_buffer && (nativeNode.buffer = saved_buffer);
      }),
      ...
      (this.disconnect = function (nodeParam) {
        ...
        else if (currNode && __.isFun(currNode.disconnect)) {
          var wrapper = getNodeWithUUID(currNode.uuid);
          try {
            currNode.disconnect();
          } catch (e) {}
          wrapper.setIsPlaying(!1), this.setIsPlaying(!1);
        }
      }),
        ...
        if (nodeToConnect && this.getUUID() !== nodeToConnect.getUUID()) {
          ...
          if (nodes.from && __.isFun(nodes.from.connect) && nodes.to) {
            var from_channel = nodes.from.from_channel,
              to_channel = nodes.from.to_channel;
            __.isNotUndef(from_channel) && __.isNotUndef(to_channel)
              ? nodes.from.connect(nodes.to, from_channel, to_channel)
              : nodes.from.connect(nodes.to);
          } else logToConsole('ERROR - connect did not happen');
        }
      }),
  ...
  function applyParam(node, keyStr, value, map) {
    for (
      ...
      i + 1 < keyArr.length && __.isNotUndef(node[keyArr[i]])
        ? 'AudioParam' === node[keyArr[i]].constructor.name
          ? setAudioParam(node[keyArr[i]], value)
          : (node = node[keyArr[i]])
        : node &&
          __.isNotUndef(node[keyArr[i]]) &&
          (node[keyArr[i]] = keyFunc(value));
  }
  function setAudioParam(node, value) {
    if (node && __.isFun(node.setValueAtTime)) {
      var time = _ignoreGrid ? _context.currentTime : _loopTimeToNextStep;
      node.cancelScheduledValues(time), node.setValueAtTime(value, time);
    }
  }
  function resolveParamMapping(name, map) {
    var mapping = map || {},
      result = mapping[name] || name;
    return result.path ? result : { path: result };
  }
            ...
            (function (params) {
              ...
              return function (stream) {
                (p.method = 'createMediaStreamSource'),
                  ...
                  getNodeWithUUID(
                    temporaryNode.getNativeNode().uuid
                  ).replaceNode(newNode);
              };
            })(params),
            function (error) {
              console.error(
                'createMediaStreamSourceNode: getUserMedia failed.'
              );
            }
          )
        : console.error(
            'createMediaStreamSourceNode: getUserMedia not supported.'
          );
  }
  function loadBuffer(userParams, node) {
    userParams && userParams.path && node
  ...
  function loadBufferWithData(dataFunction, buffersrc) {
    dataFunction && buffersrc && (buffersrc.buffer = dataFunction(_context));
  }
    ...
    path_to_soundfile &&
      ...
      fetchSoundFile(path_to_soundfile, function (sndArray) {
        _context.decodeAudioData(
          sndArray,
          function (buf) {
            (buffersrc.buffer = buf), logToConsole('sound loaded');
          },
          function (e) {
    ...
    if (path && callback) {
      ...
      request.open('GET', path, !0),
        ...
        (request.onload = function () {
          __.isFun(callback) && callback(request.response);
        }),
        request.send();
    }
  }
  function toggleGrid() {
    _isLoopRunning && (_ignoreGrid = !_ignoreGrid);
  }
  ...
  function startLoop() {
    _isLoopRunning ||
      (_loopInit(),
      (_loopTimeToNextStep = _context.currentTime + _loopInterval / 1e3),
      (_isLoopRunning = !0),
      (_ignoreGrid = !1),
      _timerWorker && _timerWorker.postMessage('start'));
  }
  function stopLoop() {
    _isLoopRunning &&
      ((_isLoopRunning = !1),
      ...
      (_ignoreGrid = !0),
      _timerWorker && _timerWorker.postMessage('stop'));
  }
  function resetLoop() {
    (_loopStepSize = 0),
      (_loopInterval = 100),
  ...
  function configureLoop(opts, fn, data) {
    opts && __.isObj(opts)
      ...
      : opts && __.isNum(opts) && !fn && !data && (_loopInterval = opts),
      __.isFun(fn) && (_loopCB = fn),
      __.isArr(data)
        ? ((_loopData = data), (_loopStepSize = data.length))
        : (_loopData = []);
  }
  function checkup() {
    if (_isLoopRunning) {
      ...
      now < _loopTimeToNextStep && now > timeAtPreviousStep
        ? (loopStep(), (_loopTimeToNextStep += loopIntervalInSecs))
        : now > _loopTimeToNextStep &&
  ...
  function _loopInit() {
    var jstext =
      ...
      blob = new Blob([jstext]),
      blobURL = window.URL.createObjectURL(blob);
    (_timerWorker = new Worker(blobURL)),
      (_timerWorker.onmessage = function (e) {
        'tick' === e.data ? checkup() : console.error('message: ' + e.data);
      }),
      _timerWorker.postMessage({ interval: calculateTimeout() });
  }
  function resetModel() {
    (_nodeStore = {}), (_nodeLookup = {});
  }
  function saveNode(node) {
    updateMacro(node), setNode(node), setNodeLookup(node);
  }
  ...
  function setNodeLookup(node) {
    ...
    selector_array.push('*'),
      setter(_nodeLookup, '*', node.getUUID()),
    ...
    function removeReferences(node) {
      ...
      if ((node = getNodeWithUUID(uuid))) {
        ...
        if (
          ...
          node.getNativeNode().forEach(function (nativeNode) {
            removeReferences(nativeNode.uuid);
          });
        }
      }
    }
    (nodes || _selectedNodes).forEach(removeReferences);
  }
  function getNodeWithUUID(uuid) {
    return uuid && _nodeStore[uuid]
      ? _nodeStore[uuid]
      : uuid && __.isObj(uuid) && 'AudioNode' === uuid.constructor.name
      ? uuid
      : __.isObj(uuid) && __.isNotUndef(uuid.uuid)
      ? getNodeWithUUID(uuid.uuid)
      : null;
  }
    ...
    return (
      selector_array.forEach(function (currSelector, i, array) {
        if (currSelector && _nodeLookup[currSelector]) {
          ...
          (tmp =
            'id' === getSelectorType(currSelector) &&
            __.isArr(tmp) &&
            tmp.length > 1
              ? [tmp[0]]
              : tmp),
  ...
  function getCurrentMacro() {
    return recordingMacro() ? _currentMacro[_currentMacro.length - 1] : null;
  }
  function getCurrentMacroNamespace() {
    var arr = [];
    if (recordingMacro())
      for (var i = 0; i < _currentMacro.length; i++)
        arr.push(_currentMacro[i].getType()), arr.push(' ');
    return arr.join('');
  }
  function resetMacro() {
    _currentMacro = [];
  ...
  function flatten(a, r) {
    ...
    for (var i = 0; i < a.length; i++)
      a[i].constructor === Array ? flatten(a[i], r) : r.push(a[i]);
    return r;
  }
  ...
  function setter(map, key, value) {
    map &&
      key &&
      (map[key] && map[key].push ? map[key].push(value) : (map[key] = [value]));
  }
  function unsetter(map, key, value) {
    __.isNotUndef(map[key]) &&
      (__.isArr(map[key])
        ? ((map[key] = map[key].filter(function (val) {
            return value !== val;
          })),
          0 === map[key].length && delete map[key])
        : delete map[key]);
  }
  function mergeObjects(source, target) {
    (source = source || {}), (target = target || {});
    for (var x in source) source.hasOwnProperty(x) && (target[x] = source[x]);
    return target;
  }
  function logNodes(node, arr) {
    if (_currentSelector) {
      var nodes = arr || [];
      (node || _selectedNodes).forEach(function (nodeID, i, array) {
        var tmp = getNodeWithUUID(nodeID).getNativeNode();
        __.isArr(tmp) && logNodes(tmp, nodes),
          array === _selectedNodes && nodes.push(tmp);
      });
    }
  }
  function logToConsole(msg) {
    _debugEnabled && console.log(msg);
  }
  ...
  (cracked.reset = function () {
    ...
    (cracked.filter = function () {
      ...
      if (arguments && arguments.length) {
        ...
        _selectedNodes.forEach(function (nodeID, i, arr) {
          ...
          (('type' === selectorType && node.getType() === match) ||
            ('class' === selectorType && node.getClass() === match) ||
            ('id' === selectorType && node.getID() === match)) &&
            tmp.push(nodeID);
        });
      }
      return tmp;
    ...
    (cracked.find = function () {
      return getNodesWithSelector(
        recordingMacro() ? processSelectorForMacro(arguments[0]) : arguments[0]
      );
    }),
      ...
      function _start() {
        if (!recordingMacro())
          for (var i = 0; i < _selectedNodes.length; i++) {
            ...
            currNode && !currNode.getIsPlaying() && currNode.start();
          }
      }
      return (
    ...
    (cracked.stop = function () {
      for (var i = 0; i < _selectedNodes.length; i++) {
        var currNode = getNodeWithUUID(_selectedNodes[i]);
        currNode && currNode.getIsPlaying() && currNode.stop(0);
      }
      return cracked;
    }),
      ...
      for (
        var mapParam = function (node, param) {
            ...
            if (node.getParamMapping) {
              (mappingResult = (node.getParamMapping() || {})[param] || ''),
                -1 !== mappingResult.indexOf('.') &&
                  (mappingResult = mappingResult.split('.')[0]);
            }
            return mappingResult;
          },
          i = 0;
        i < _selectedNodes.length;
        i++
      ) {
        for (
          var currNode = getNodeWithUUID(_selectedNodes[i]),
            native = currNode.isMacro()
    ...
    (cracked.attr = function (userParams) {
      if ('object' == typeof userParams)
        for (var i = 0; i < _selectedNodes.length; i++) {
          var currNode = getNodeWithUUID(_selectedNodes[i]);
          currNode && userParams && currNode.attr(userParams);
        }
      else if (
        'string' == typeof userParams &&
    ...
    (cracked.script = function (userParams) {
      ...
      var buffersize = userParams.buffersize || 4096,
        channels = userParams.channels || 1,
        fn = userParams.fn || defaultFunction;
      return (
        createNode(
          'script',
          {
      ...
      return (
        createNode(
          'gain',
          {
            method: 'createGain',
            settings: {},
            mapping: { gain: 'gain.value' },
          },
          __.isObj(userParams) ? userParams : { gain: gain }
        ),
        cracked
      );
    }),
      ...
      return (
        createNode(
          'osc',
          {
            ...
            settings: {},
            mapping: { frequency: 'frequency.value', detune: 'detune.value' },
          },
          userParams
        ),
        cracked
    ...
    (cracked.convolver = function (userParams) {
      return (
        ...
        loadBuffer(
          ...
          createNode(
            'convolver',
            { method: 'createConvolver', settings: {} },
            userParams
          )
        ),
        cracked
    ...
    (cracked.destination = function (userParams) {
      return (
        createNode(
          'destination',
          { method: 'createDestination', settings: {} },
          userParams
        ),
        cracked
      );
    ...
    (cracked.origin = function (userParams) {
      var cParams = { method: 'createOrigin', settings: {} };
      return (
        createMediaStreamSourceNode(
          cParams,
          createNode('origin', cParams, userParams)
        ),
        cracked
      );
    }),
  ...
  (cracked.loop = function () {
    return (
      arguments &&
        ...
        ('stop' === arguments[0]
          ...
          : 1 === arguments.length && __.isNum(arguments[0])
          ? configureLoop(arguments[0])
          : 1 === arguments.length && __.isObj(arguments[0])
          ? configureLoop(arguments[0])
          : 2 === arguments.length &&
    ...
    (cracked.unbind = function (eventType) {
      if ('step' === eventType) {
        ...
        _loopListeners.forEach(function (el, i, arr) {
          _currentSelector !== el.selector && tmp.push(el);
        }),
          (_loopListeners = tmp);
      }
    }),
    (cracked.removeMacros = function () {
      for (var arr = _currentSelector.split(','), i = 0; i < arr.length; i++)
        if (_nodeLookup[arr[i]])
          for (
            var keyArr = Object.keys(_nodeLookup), j = 0;
            j < keyArr.length;
            j++
          ) {
    ...
    (cracked.begin = function (name, userParams) {
      return name && _currentMacro.push(createMacro(name, userParams)), cracked;
    }),
  ...
  (cracked.midi_supported = function () {
    ...
    (cracked.midi_init = function (callback) {
      _midi_access
        ? callback.apply(cracked, [!0])
        : __.midi_supported()
        ? navigator.requestMIDIAccess().then(
            function (access) {
              (_midi_access = access),
                (_midi_inputs = access.inputs),
                (_midi_outputs = access.outputs),
                callback.apply(cracked, [!0]);
            },
            function (err) {
              console.error(
                ...
                callback.apply(cracked, [!1]);
            }
          )
        : (console.error(
            'midi_init: Failed. Midi not supported by your browser'
          ),
          callback.apply(cracked, [!1]));
    }),
    (cracked.midi_receive = function (callback) {
      ...
      else console.error('midi_receive: midi not available');
    }),
    (cracked.midi_noteon = function (callback) {
      __.isFun(callback) &&
        ((_midi_callbacks.noteon = callback), cracked.midi_receive());
    }),
    (cracked.midi_noteoff = function (callback) {
      __.isFun(callback) &&
        ((_midi_callbacks.noteoff = callback), cracked.midi_receive());
    }),
    (cracked.midi_control = function (callback) {
      __.isFun(callback) &&
        ((_midi_callbacks.control = callback), cracked.midi_receive());
    }),
    ...
    (cracked.remove = function (time) {
      function _remove(nodes) {
        nodes.forEach(function (node, i, array) {
          (node = getNodeWithUUID(node)) && (node.stop(), node.disconnect());
        }),
          removeModelReferences(nodes);
      }
      var nodesToRemove = _selectedNodes.slice(),
        when = __.isNum(time) ? time : 0;
      when
        ? setTimeout(function () {
    ...
    (cracked.log = function () {
      var arr = [];
      logNodes(null, arr), console.log(arr);
    }),
    (cracked.size = function () {
      return _selectedNodes.length ? _selectedNodes.length : 0;
    }),
    (function () {
      -1 !== window.location.href.indexOf('debug=true') && (_debugEnabled = !0);
    })(),
    ...
    (cracked.isNum = function (num) {
      return null !== num && '' !== num && void 0 !== num && !isNaN(num);
    }),
    ...
    (cracked.random_interval = function (callback, minTime, maxTime) {
      function set_timeout(callback, minTime, maxTime, ran) {
        ...
        setTimeout(function () {
          callback(nextRan), set_timeout(callback, minTime, maxTime, nextRan);
        }, ran);
      }
      'function' == typeof callback &&
        ...
        set_timeout(
          ...
          minTime,
          maxTime,
          cracked.random(minTime, maxTime)
        );
    }),
    ...
    (cracked.array_next = function (arr, offset, limit, callback) {
      (offset = offset || 0), (limit = limit || arr.length);
      var adjusted_limit = Math.min(limit, arr.length),
        adjusted_offset = Math.min(offset, adjusted_limit - 1),
        old_index = (arr.current_index = __.ifUndef(arr.current_index, -1));
      return (
        (arr.current_index =
          arr.current_index + 1 + adjusted_offset >= adjusted_limit
            ? 0
            : arr.current_index + 1),
        old_index > arr.current_index &&
          'function' == typeof callback &&
          callback(),
        arr[arr.current_index + adjusted_offset]
      );
    }),
    ...
    (cracked.random_scale = function (scale, octave_lower, octave_upper) {
      var lower = __.ifUndef(octave_lower, 3),
        upper = __.ifUndef(octave_upper, 7);
      return __.pitch2freq(
        __.scales(scale)[__.random(0, __.scales(scale).length - 1)] +
          12 * __.random(lower, upper)
      );
    }),
    (cracked.random_arpeggio = function (chord, octave_lower, octave_upper) {
      var lower = __.ifUndef(octave_lower, 3),
        upper = __.ifUndef(octave_upper, 7);
      return __.pitch2freq(
        __.chords(chord)[__.random(0, __.chords(chord).length - 1)] +
          12 * __.random(lower, upper)
      );
    }),
    (cracked.shuffle = function (arr) {
      for (var temp, index, counter = arr.length; counter > 0; )
    ...
    (cracked.sequence_factory = function (arr, fn) {
      ...
      return function () {
        count === current_value
          ? (current_index === arr_copy.length - 1
              ? (current_index = 0)
              : current_index++,
            (count = 1),
            (transition = !0),
            (current_value = arr_copy[current_index]))
          : (count++, (transition = first_run || !1)),
    ...
    (cracked.sequence = function (name) {
      return (
        cracked.__sequence_storage[name] ||
          (cracked.__sequence_storage[name] = { steps: {} }),
        (function (name) {
          return {
            step: function (func, time) {
              return (
                (cracked.__sequence_storage[name].steps[time.toString()] =
                  func),
                this
              );
            },
            start: function () {
              (cracked.__sequence_storage[name].startTime = Date.now()),
                (cracked.__sequence_storage[name].clearInterval = setInterval(
                  function () {
                    var time_elapsed =
                        Date.now() - cracked.__sequence_storage[name].startTime,
                      steps = Object.keys(
                        cracked.__sequence_storage[name].steps
                      );
                    steps.length
                      ? steps.map(function (x) {
                          Math.floor(time_elapsed / 6e4) >= x &&
                            (cracked.__sequence_storage[name].steps[x](),
                            delete cracked.__sequence_storage[name].steps[x]);
                        })
                      : clearInterval(
                          cracked.__sequence_storage[name].clearInterval
                        );
                  },
                  1e3
                ));
      ...
      function buildImpulse(audioContext) {
        ...
        for (i = 0; i < length; i++)
          (n = _reverse ? length - i : i),
            (impulseL[i] =
              (2 * Math.random() - 1) * Math.pow(1 - n / length, decay)),
            (impulseR[i] =
              (2 * Math.random() - 1) * Math.pow(1 - n / length, decay));
        return impulse;
      }
      (params = __.ifUndef(params, {})),
        params.path || (params.fn = params.fn || buildImpulse);
      var _seconds = __.ifUndef(params.seconds, 3),
        _reverse = __.ifUndef(params.reverse, !1),
        _decay = __.ifUndef(params.decay, 2);
      return (
        __.begin('reverb', params).convolver(params).end('reverb'), cracked
      );
    }),
      ...
      return (
        ...
        __.gain({ id: 'delay-input' })
          ...
          .gain({
            ...
            mapping: { damping: 'gain.value' },
          })
          .lowpass({
            id: 'delay-cutoff',
            frequency: __.ifUndef(params.cutoff, 1500),
            mapping: { cutoff: 'frequency.value' },
          })
          .gain({
            id: 'delay-feedback',
            gain: __.ifUndef(params.feedback, 0.5),
            mapping: { feedback: 'gain.value' },
          })
          .gain({ id: 'delay-output' }),
    ...
    (cracked.comb = function (params) {
      ...
      return (
        ...
        __.gain({ id: 'comb-input' })
          ...
          .lowpass({
            id: 'comb-cutoff',
            frequency: __.ifUndef(userParams.cutoff, 3e3),
            mapping: { cutoff: 'frequency.value' },
          })
          .gain({
            id: 'comb-feedback',
          ...
          .connect('#comb-input'),
        __('#comb-damping').gain({ id: 'output' }),
        __.end('comb'),
        cracked
      );
    }),
      ...
      return (
        ...
        __.script({
          fn: (function (options) {
            ...
            var bits = options.bits || 6,
              normfreq = __.ifUndef(options.frequency, 0.1),
          ...
          (v = (i - samples / 2) / (samples / 2)),
            ...
            (value =
              v <= vb
                ...
                : h * v -
                  h * vl +
                  h * (Math.pow(vl - vb, 2) / (2 * vl - 2 * vb))),
            (wsCurve[i] = value);
        return wsCurve;
      }
      var options = params || {},
        thisCurve = setCurve(__.ifUndef(options.distortion, 1));
      return (
        __.begin('ring', params),
        __.gain({ id: 'player' })
          .gain({ id: 'vcInverter1', gain: -1 })
          .waveshaper({
            id: 'vcDiode3',
            curve: thisCurve,
            mapping: {
        ...
        __()
          .sine({
            ...
            mapping: { frequency: 'frequency.value' },
          })
          .gain({ id: 'vInGain', gain: 0.5 })
          .gain({ id: 'vInInverter1', gain: -1 })
          .gain({ id: 'vInInverter2', gain: -1 })
          .waveshaper({
            id: 'vInDiode1',
            curve: thisCurve,
            mapping: {
        ...
        __('compressor').gain({ id: 'outGain', gain: 4 }),
        __.end('ring'),
        cracked
      );
    }),
    (cracked.overdrive = function (params) {
      ...
      return (
        ...
        __.gain({ id: 'input' })
          ...
          .lowpass({
            frequency: __.ifUndef(params.postCut, 3e3),
            mapping: { postCut: 'frequency.value' },
          })
          .gain({ id: 'output' }),
        __.end('overdrive'),
        cracked
    ...
    (cracked.adsr = function (userParams) {
      function makeEnv(userParams, nodeParams) {
        var segment,
          args = userParams || nodeParams,
          p = [0, 0, 0, 0, 0],
          options = args || 0.5;
        if (__.isArr(options))
          if (5 === options.length) p = options;
          else if (4 === options.length) {
          ...
          __.isNum(options)
            ...
            : __.isStr(options) &&
              ((options =
                'slow' === options ? 1 : 'fast' === options ? 0.25 : 0.5),
              (segment = options / 4),
              (p = [segment, segment, 0.5, segment, segment]));
        return p;
      }
          ...
          (options = options || {}),
            (options =
              __.isNum(options) || __.isArr(options) || __.isStr(options)
                ? { envelope: options }
                : options),
            __.begin('adsr', options).gain({ gain: 0 }).end('adsr');
        },
    ...
    (cracked.lowpass = function (params) {
      var freq = __.isNum(params) ? params : 440,
        userParams = __.isObj(params) ? params : {},
        options = {};
      return (
    ...
    (cracked.mouse_movement = function (callback) {
      ...
      window.parent.document.removeEventListener('mousemove', moveHandler, !1),
        window.parent.document.addEventListener('mousemove', moveHandler, !1);
    }),
    ...
    (cracked.multi_out = function (params) {
      var to_channel = __.isNum(params)
          ...
          : 0,
        userParams = __.isObj(params) ? params : {},
        gain = userParams.gain ? userParams.gain : 1;
      return (
        (userParams.mapping = userParams.mapping || {}),
      ...
      return (
        ...
        'brown' === params.type
          ? __.begin('lfo', params)
              ...
              .gain({ gain: __.ifUndef(params.gain, 1e3) })
              .end('lfo')
          : __.begin('lfo', params)
              .osc({
    ...
    (cracked.stepper = function (params) {
      function buildBuffer(audioContext) {
        var buffer = audioContext.createBuffer(
          ...
          buffArr = [],
          stepSize = Math.floor(buflen / steps),
          stepCount = 0,
          value = 0;
        buffArr.push(buffer.getChannelData(0));
      ...
      var userParams = params || {},
        length = userParams.length || 1,
        steps = userParams.steps || 8,
        fn =
          userParams.fn ||
          function () {
      ...
      var step_size = length / steps,
        bufferParams = { fn: buildBuffer, loop: !0 },
        start_point = userParams.start * step_size || 0,
        end_point = userParams.end * step_size || 0;
      return (
        start_point && (bufferParams.start = start_point),
        end_point && (bufferParams.end = end_point),
        __()
    ...
    (cracked.pink = function (params) {
      function buildBuffer(audioContext) {
        function pinkify(buf, buffArr) {
          ...
          for (i = 0; i < bufNum; i++) {
            for (
              ...
              (white = channelData[j]),
                ...
                (b[2] = 0.969 * b[2] + 0.153852 * white),
                (b[3] = 0.8665 * b[3] + 0.3104856 * white),
                (b[4] = 0.55 * b[4] + 0.5329522 * white),
                (b[5] = -0.7616 * b[5] - 0.016898 * white),
                (pink[i][j] =
                  b[0] +
                  b[1] +
                  b[2] +
          ...
          for (i = 0; i < bufNum; i++)
            for (j = 0; j < buflen; j++) buffArr[i][j] = pink[i][j];
        }
        for (
        ...
        for (var i = 0; i < buflen; i++)
          for (var j = 0; j < bufNum; j++)
            buffArr[j][i] = 2 * Math.random() - 1;
        return pinkify(buf, buffArr), buf;
      }
      var userParams = params || {},
        channels = userParams.channels || 1,
        length = userParams.length || 1;
    ...
    (cracked.white = function (params) {
      function buildBuffer(audioContext) {
        ...
        for (var i = 0; i < buflen; i++)
          for (var j = 0; j < bufNum; j++)
            buffArr[j][i] = 0.44 * (2 * Math.random() - 1);
        return buffer;
      }
      var userParams = params || {},
        channels = userParams.channels || 1,
    ...
    (cracked.frequency = function (userParam) {
      return (
        __.isNum(userParam) && cracked.attr({ frequency: userParam }), cracked
      );
    }),
    (cracked.detune = function (userParam) {
      return (
        __.isNum(userParam) && cracked.attr({ detune: userParam }), cracked
      );
    }),
    (cracked.type = function (userParam) {
      return __.isStr(userParam) && cracked.attr({ type: userParam }), cracked;
    }),
    (cracked.volume = function (userParam) {
      return __.isNum(userParam) && cracked.attr({ gain: userParam }), cracked;
    }),
    (cracked.fadeOut = function (userParam) {
      var num = __.isNum(userParam) ? userParam : 0.02;
      return cracked.ramp(0, num, 'gain'), cracked;
    }),
    (cracked.fadeIn = function (userParam) {
      var num = __.isNum(userParam) ? userParam : 0.02;
      return cracked.ramp(1, num, 'gain'), cracked;
    }),
    (cracked.time = function (userParam) {
      return __.isNum(userParam) && cracked.attr({ delay: userParam }), cracked;
    }),
    (cracked.feedback = function (userParam) {
      return (
        __.isNum(userParam) && cracked.attr({ feedback: userParam }), cracked
      );
    }),
    (cracked.speed = function (userParam) {
      return __.isNum(userParam) && cracked.attr({ speed: userParam }), cracked;
    }),
    (cracked.drive = function (userParam) {
      return __.isNum(userParam) && cracked.attr({ drive: userParam }), cracked;
    }),
    (cracked.distortion = function (userParam) {
      return (
        __.isNum(userParam) && cracked.attr({ distortion: userParam }), cracked
      );
    }),
    (cracked.q = function (userParam) {
      return __.isNum(userParam) && cracked.attr({ q: userParam }), cracked;
    }),
    (cracked.pan = function (userParam) {
      return __.isNum(userParam) && cracked.attr({ pan: userParam }), cracked;
    }),
    (cracked.gang_of_oscillators = function (params) {
      var type = __.isStr(params)
          ...
          ? params.type
          : 'sine',
        lp_freq = __.isObj(params) && params.lp_freq ? params.lp_freq : 2e4,
        size = __.isObj(params) && params.size ? params.size : 16;
      __().begin('gang_of_oscillators', params), __().gain();
      for (var i = 0; i < size; i++)
        __()
          ...
          .end('v' + (i + 1))
          .connect('gain');
      return __.end('gang_of_oscillators'), cracked;
    }),
    (cracked.monosynth = function (params) {
      var methods = {
        init: function (options) {
          var opts = options || {},
            lfo_type = opts.lfo_type || 'sawtooth',
          ...
          __()
            ...
            .lfo({ gain: lfo_intensity, frequency: lfo_speed, type: lfo_type })
            .osc({
              detune: osc_detune,
              frequency: osc_frequency,
              type: osc_type,
            })
            .lowpass({ q: lp_q, frequency: lp_frequency })
            .adsr({ envelope: adsr_envelope })
            .gain({ gain: gain_volume })
            .end('monosynth');
        },
        noteOn: function (params) {
          var args = params || {},
            freq = __.isNum(params)
              ? __.pitch2freq(params)
              : __.pitch2freq(args.pitch),
            vel = __.isNum(args.velocity) ? args.velocity / 127 : 0.5,
            env = args.envelope || [0.01, 0.1, 0.5];
        ...
        noteOff: function (params) {
          cracked.each('monosynth', function (el, index, arr) {
            ...
            var p = __.isNum(params)
              ? params
              : __.ifUndef(params.envelope, 0.1);
        ...
        init: function (options) {
          ...
          (params = params || {}),
            ...
            __()
              ...
              .gain({ gain: params.gain_volume })
              .end('polysynth');
        },
        noteOn: function (params) {
          var args = params || {},
            note_number = __.isNum(params) ? params : args.pitch,
            freq = __.pitch2freq(note_number),
            vel = __.isNum(args.velocity) ? args.velocity / 127 : 0.5,
            env = args.envelope || [0.01, 0.1, 0.5],
            instance_id = note_number + '_' + Date.now();
          cracked.each('polysynth', function (el, index, arr) {
            var voices = el.getParams().settings.active_voices,
              settings = el.getParams().settings;
            voices[note_number] ||
              (__.loop('toggle_grid'),
              __()
                .lfo({
        ...
        noteOff: function (params) {
          cracked.each('polysynth', function (el, index, arr) {
            var note_number = __.isNum(params) ? params : params.pitch,
              release = __.ifUndef(params.envelope, 0.1),
              voices = el.getParams().settings.active_voices,
              instance_id = !!note_number && voices[note_number];
            instance_id &&
              (cracked.exec(
                'adsr',
                ['release', release],
    ...
    (cracked.play = function () {
      return cracked('*').start(), cracked;
    }),
    (cracked.scale = function (position, inMin, inMax, outMin, outMax, type) {
      if ('log' === type || 'logarithmic' === type) {
        var minVal = Math.log(outMin || 1),
          maxVal = Math.log(outMax || 1),
          scale = (maxVal - minVal) / (inMax - inMin);
        return Math.exp(minVal + scale * (position - inMin));
      }
      if ('linear' === type || void 0 === type) {
        return parseFloat(
          ((position - inMin) * (outMax - outMin)) / (inMax - inMin) + outMin
        ).toFixed(2);
      }
      return console.error('scale: type ' + type + ' not supported.'), position;
    }),
    (cracked.sec2ms = function (second) {
      return __.isNum(second)
        ? 1e3 * second
        : (console.error('sec2ms: param not number'), second);
    }),
    (cracked.ms2sec = function (ms) {
      return __.isNum(ms)
        ? ms / 1e3
        : (console.error('ms2sec: param not number'), ms);
    }),
    (cracked.ms2min = function (ms) {
      return __.isNum(ms)
        ? ms / 1e3 / 60
        : (console.error('ms2min: param not number'), ms);
    }),
    (cracked.min2ms = function (minute) {
      return __.isNum(minute)
        ? 1e3 * minute * 60
        : (console.error('min2ms: param not number'), minute);
    }),
