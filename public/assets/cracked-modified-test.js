  ...
  if (arguments && arguments.length) {
    if (recordingMacro()) {
      ...
      if (__.isStr(arguments[0])) {
        ...
        __.isObj(arguments[0]) &&
        arguments[0].constructor.name === 'AudioNode'
      ) {
...
function findInMacro() {
  if (arguments && arguments.length) {
    if (__.isStr(arguments[0])) {
      ...
      _currentSelector =
        getCurrentMacroNamespace() + ' ' + arguments[0].getType();
      _selectedNodes = [arguments[0].getUUID()];
    }
  }
}
...
function processSelectorForMacro(selector) {
  ...
  for (var i = 0; i < selectorArr.length; i++) {
    selectorArr[i] =
      selectorArr[i].indexOf(prefix) !== -1
        ? selectorArr[i]
        : prefix + selectorArr[i];
  }
...
cracked.find = function () {
  var selector = recordingMacro()
    ? processSelectorForMacro(arguments[0])
    : arguments[0];
...
function createNode(type, creationParams, userSettings) {
  ...
  var nodesToConnect = _selectedNodes.length
    ? _selectedNodes
    : [getPreviousNode()];
...
function audioNodeFactory(creationParams) {
  ...
  if (_context && creationParams.method && _context[creationParams.method]) {
    node = _context[creationParams.method].apply(
      _context,
      creationParams.methodParams || []
    );
    for (var creationParam in creationParams.settings) {
      if (creationParams.settings.hasOwnProperty(creationParam)) {
        if (
          creationParam === 'to_channel' ||
          creationParam === 'from_channel'
        ) {
          ...
          applyParam(
            node,
            creationParam,
            creationParams.settings[creationParam],
            creationParams.mapping
          );
        }
      }
    }
    ...
    throw new Error(
      "Couldn't create audio node. Create method " +
        creationParams.method +
        ' not supported.'
    );
  }
  logToConsole(node);
...
function AudioNode(type, creationParams, userSettings) {
    ...
    if (__.isArr(currNode)) {
      ...
      if (currNode && __.isFun(currNode.start)) {
        ...
        if (!wrapper.getIsPlaying()) {
          var offset =
            currNode && currNode.loopStart && __.isNum(currNode.loopStart)
              ? currNode.loopStart
              : 0;
          var duration =
            currNode && currNode.loopEnd && __.isNum(currNode.loopEnd)
              ? currNode.loopEnd - offset
              : 0;
        ...
        if (
          ...
          for (var i = 0; i < target.length; i++) {
            ...
            logToConsole(
              ' target ' +
                target[i] +
                ' time ' +
                (_context.currentTime + prevTime + time[i]) +
                ' current time ' +
                _context.currentTime
            );
            _target = target[i] === 0 && type === 'exp' ? 0.0001 : target[i];
            currNode[paramToRamp][rampMethod](
              _target,
              now + prevTime + time[i]
            );
          }
          ...
          logToConsole(
            ' target ' +
              target +
              ' time ' +
              (_context.currentTime + prevTime + time) +
              ' current time ' +
              _context.currentTime
          );
        ...
        if (__.isNotUndef(userParams[key])) {
          ...
          if (__.isArr(nativeNode)) {
            flatten(nativeNode).forEach(function (_node, _i, _array) {
              var mapping =
                userParams.mapping ||
                getNodeWithUUID(_node.uuid).getParamMapping();
            ...
            var mapping =
              userParams.mapping ||
              getNodeWithUUID(nativeNode.uuid).getParamMapping();
  ...
  this.search = function (str) {
    ...
    if (str) {
      ...
      for (var i = 0; i < nodeIds.length; i++) {
        if (
          getNodeWithUUID(nodeIds[i]).getMacroContainerUUID() === this.getUUID()
        ) {
  ...
  this.areSiblings = function (node) {
    ...
    return (
      ...
      parentNodeID === this.getMacroContainerUUID()
    );
  };
  ...
  this.isModulator = function () {
    if (
      this.getMacroContainerUUID() &&
      !!getNodeWithUUID(this.getMacroContainerUUID()).getParams().settings
        .modulates
    ) {
      return true;
    } else {
  ...
  this.isModulatorType = function () {
    if (
      ...
      return getNodeWithUUID(this.getMacroContainerUUID()).getParams().settings
        .modulates;
  ...
  this.resetNode = function (currNode) {
    ...
    if (this.isMacro()) {
      ...
      updateNativeNode(
        getNodeWithUUID(currNode.uuid).getNativeNode(),
        currNode.uuid
      );
    } else {
  ...
  function getNodeToConnect(node, whichNode) {
    if (node) {
      ...
      if (__.isArr(rawNode)) {
        ...
        rawNode =
          whichNode === 'from'
            ? getRawNode(rawNode, rawNode.length - 1)
            : getRawNode(rawNode, 0);
      }
      return rawNode;
    } else {
      return null;
...
cracked.ramp = function (target, timeToRamp, paramToRamp, initial, type) {
  ...
  for (var i = 0; i < _selectedNodes.length; i++) {
    var currNode = getNodeWithUUID(_selectedNodes[i]);
    var native = currNode.isMacro()
      ? currNode.getNativeNode()
      : [currNode.getNativeNode()];
    var mappedParam = '';
...
cracked.attr = function (userParams) {
  if (typeof userParams === 'object') {
    ...
    typeof userParams === 'string' &&
    _selectedNodes.length &&
    getNodeWithUUID(_selectedNodes[0])
  ) {
...
cracked.gain = function (userParams) {
  ...
  var params = __.isObj(userParams)
    ? userParams
    : {
        gain: gain,
      };
...
cracked.channelMerger = function (userParams) {
  ...
  var channels = __.isNum(userParams)
    ? userParams
    : __.isObj(userParams) && userParams.channels
    ? userParams.channels
    : _context.destination.maxChannelCount;
  var creationParams = {
...
function createMockMediaStream(creationParams) {
  ...
  var tmpnode = _context[creationParams.method].apply(
    _context,
    creationParams.methodParams || []
  );
  for (var creationParam in creationParams.settings) {
    ...
    navigator.getUserMedia(
      ...
      (function (params) {
        ...
        return function (stream) {
          ...
          getNodeWithUUID(temporaryNode.getNativeNode().uuid).replaceNode(
            newNode
          );
        };
      })(params),
...
cracked.buffer = function (userParams) {
  ...
  loadBuffer(userParams, buffersrc, userParams.onLoadBuffer);
  return cracked;
};
 ...
 * @private
 */
function loadBuffer(userParams, node, callback) {
  if (userParams && userParams.path && node) {
    loadBufferFromFile(userParams.path, node.getNativeNode(), callback);
 ...
 * @private
 */
function loadBufferFromFile(path_to_soundfile, buffersrc, callback) {
...
cracked.loop = function () {
  if (arguments && arguments.length) {
    if (arguments[0] === 'stop') {
      ...
      arguments.length === 3 &&
      __.isObj(arguments[0]) &&
      __.isFun(arguments[1]) &&
      __.isArr(arguments[2])
    ) {
...
function calculateTimeout() {
  return __.sec2ms(
    _loopTimeToNextStep -
      _context.currentTime -
      __.ms2sec(_loopInterval * _loopTolerance)
  );
}
...
function loopStep() {
  ...
  if (__.isFun(_loopCB)) {
    _loopCB(
      _loopIndex,
      cracked.ifUndef(_loopData[_loopIndex], null),
      _loopData,
      ++_loopCount
    );
  }
  ...
  for (var i = 0; i < _loopListeners.length; i++) {
    ...
    listener.callback(
      listener.loopIndex,
      cracked.ifUndef(data[listener.loopIndex], null),
      data,
      count
    );
...
function _loopInit() {
  ...
  //prepare worker blob
  var jstext =
    'var timerID=null,interval=100;self.onmessage=function(a){"start"==a.data?timerID=setInterval(function(){postMessage("tick")},interval):a.data.interval?(interval=a.data.interval,timerID&&(clearInterval(timerID),timerID=setInterval(function(){postMessage("tick")},interval))):"stop"==a.data&&(clearInterval(timerID),timerID=null)};';
...
function getNodeWithUUID(uuid) {
  ...
  } else if (uuid && __.isObj(uuid) && uuid.constructor.name === 'AudioNode') {
...
cracked.end = function (name) {
  if (recordingMacro() && getCurrentMacro().getType() === name) {
...
cracked.midi_init = function (callback) {
  if (_midi_access) {
    ...
    navigator.requestMIDIAccess().then(
      ...
      function (err) {
        console.error(
          'midi_init: Initialization failed. Error code: ' + err.code
        );
...
cracked.scale = function (position, inMin, inMax, outMin, outMax, type) {
  if (type === 'log' || type === 'logarithmic') {
    ...
    var result = parseFloat(
      ((position - inMin) * (outMax - outMin)) / (inMax - inMin) + outMin
    );
...
cracked.ring = function (params) {
  ...
  __('#vInGain').connect('#vInDiode4');

  __('#vInGain').connect('#vcInverter1');
...
cracked.adsr = function (userParams) {
  var methods = {
    init: function (options) {
      ...
      options =
        __.isNum(options) || __.isArr(options) || __.isStr(options)
          ? { envelope: options }
          : options;

      __.begin('adsr', options)
        .gain({
          gain: 0,
        })
        .end('adsr');
    ...
    trigger: function (params) {
      cracked.each('adsr', function (el, i, arr) {
        ...
        el.ramp([1, p[2], p[2], 0], [p[0], p[1], p[3], p[4]], 'gain', null, 0);
      });
    },
...
cracked.multi_out = function (params) {
  var to_channel = __.isNum(params)
    ? params
    : __.isObj(params) && params.channel
    ? params.channel
    : 0;
  var userParams = __.isObj(params) ? params : {};
  var gain = userParams.gain ? userParams.gain : 1;
  userParams.mapping = userParams.mapping || {};
  __.begin('multi_out', userParams)
    .gain({ from_channel: 0, to_channel: to_channel, gain: gain })
    .channelMerger()
    .destination()
    .end('multi_out');
  return cracked;
...
cracked.panner = function (params) {
  var pan = __.isNum(params)
    ? params
    : __.isObj(params) && params.pan
    ? params.pan
    : 0;
  var userParams = __.isObj(params) ? params : {};
  var options = {};
...
cracked.stepper = function (params) {
  ...
  function buildBuffer(audioContext) {
    ...
    for (var i = 0; i < buflen; i++) {
      value =
        !i || (i % stepSize === 0 && stepCount < steps)
          ? (function () {
              stepCount++;
              return fn();
            })()
...
cracked.pink = function (params) {
  ...
  function buildBuffer(audioContext) {
    var buf = audioContext.createBuffer(
      channels,
      length * audioContext.sampleRate,
      audioContext.sampleRate
    );
    ...
    function pinkify(buf, buffArr) {
      var buffer = buf,
        ...
        j,
        pink = [],
        bufNum = buffer.numberOfChannels,
...
cracked.gang_of_oscillators = function (params) {
  ...
  var type = __.isStr(params)
    ? params
    : __.isObj(params) && params.type
    ? params.type
    : 'sine';
...
cracked.monosynth = function (params) {
  var methods = {
    ...
    noteOn: function (params) {
      ...
      var args = params || {};
      var freq = __.isNum(params)
        ? __.pitch2freq(params)
        : __.pitch2freq(args.pitch);
